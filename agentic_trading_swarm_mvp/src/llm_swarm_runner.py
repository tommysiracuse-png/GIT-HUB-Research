...
def parse_recommendation(text: str, agent: dict, packet: dict) -> dict:
    ...
    if rec is None:
        parsed_top_level = _top_level_json_object(text)
        if (
            agent.get("name") == "cross_market_researcher"
            and isinstance(parsed_top_level, dict)
            and _describes_no_change(parsed_top_level)
        ):
            return _reject_recommendation(
                agent,
                str(
                    parsed_top_level.get("rationale")
                    or parsed_top_level.get("proposed_change")
                    or parsed_top_level.get("title")
                    or "Recommendation describes no actionable change."
                ),
                "native_valid",
    ...
    if not str(rec.get("title") or "").strip():
        rec["title"] = f"{agent['name']} recommendation"
    if not str(rec.get("rationale") or "").strip():
        rec["rationale"] = "Generated by LLM swarm."
    if not str(rec.get("market_key") or "").strip():
        rec["market_key"] = agent["name"]
    rec["evidence"] = _normalize_recommendation_mapping(
        rec.get("evidence"),
        fallback=str(rec.get("rationale") or rec.get("title") or "Model returned no structured evidence."),
        source_format_hint="string" if isinstance(rec.get("evidence"), str) else None,
    )
    rec["proposed_change"] = _normalize_recommendation_mapping(
        rec.get("proposed_change"),
        fallback=str(rec.get("rationale") or rec.get("title") or "Review recommendation."),
    )
...
def _has_non_empty_recommendation_value(value: Any) -> bool:
    if isinstance(value, str):
        return bool(value.strip())
    if isinstance(value, dict):
        return any(_has_non_empty_recommendation_value(item) for item in value.values())
    return value not in (None, [], {}, ())


def _normalize_recommendation_mapping(
    value: Any,
    *,
    fallback: str,
    ...
    if isinstance(value, dict):
        ...
        if source_format_hint:
            normalized["source_format"] = source_format_hint
    elif isinstance(value, list):
        normalized = {"items": value, "source_format": "list"}
    else:
        normalized = {}
    if _has_non_empty_recommendation_value(normalized):
        return normalized
    text = str(fallback or "").strip() or "Keep the recommendation paper-only and auditable."
    return {"summary": text}
...
def _top_level_json_object(text: str) -> dict[str, Any] | None:
    raw = (text or "").strip()
    if not raw:
        return None
    try:
