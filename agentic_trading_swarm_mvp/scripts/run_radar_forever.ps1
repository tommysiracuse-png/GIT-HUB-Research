...
$ExpectedEntryPoint = [System.IO.Path]::GetFullPath((Join-Path $ProjectRoot "src\radar_loop.py"))
$ProjectPrefix = $ProjectRoot.TrimEnd([System.IO.Path]::DirectorySeparatorChar) + [System.IO.Path]::DirectorySeparatorChar
if (-not [string]::Equals($RadarEntryPoint, $ExpectedEntryPoint, [System.StringComparison]::OrdinalIgnoreCase) -or
    -not $RadarEntryPoint.StartsWith($ProjectPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "Radar entry point is outside the exact project workspace: $RadarEntryPoint"
}
...
$PidMetaPath = Join-Path $RunsDir "radar_forever.pid.json"
$HeartbeatPath = Join-Path $RunsDir "radar_heartbeat.json"
$ChildStdoutPath = Join-Path $RunsDir "radar_child_$PID.stdout.tmp"
$ChildStderrPath = Join-Path $RunsDir "radar_child_$PID.stderr.tmp"
$MaxLogBytes = 10MB
$HeartbeatIntervalSeconds = 15
$IterationIntervalSeconds = 60
$MaxIterationRuntimeMinutes = 240
$child = $null
$childStartedAt = ""
...
function Test-ExistingOwner {
    foreach ($process in @(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue)) {
        ...
        if ([string]$process.Name -like "python*.exe" -and
            (Test-ExactPathArgument -CommandLine ([string]$process.CommandLine) -ExpectedPath $RadarEntryPoint)) { return $true }
    }
    return $false
}
...
function Stop-OwnedChildTree {
    ...
    if ($root -and ([string]$root.Name -notlike "python*.exe" -or
        ...
        -not (Test-ExactPathArgument -CommandLine ([string]$root.CommandLine) -ExpectedPath $RadarEntryPoint) -or
...
$MutexName = "Global\AgenticTradingSwarm.Radar.$(Get-RootHash -Root $ProjectRoot)"
...
try {
    ...
    # Losing contenders exit immediately after failing the mutex. Give them a
    # short grace period before deciding an exact-path legacy owner remains.
    $existingOwner = $false
    for ($attempt = 0; $attempt -lt 8; $attempt++) {
        $existingOwner = Test-ExistingOwner
        if (-not $existingOwner) { break }
        Start-Sleep -Milliseconds 250
    }
    if ($existingOwner) { exit 0 }
    ...
    function Rotate-Log-IfNeeded {
        if ((Test-Path -LiteralPath $LogPath) -and (Get-Item -LiteralPath $LogPath).Length -gt $MaxLogBytes) {
            $archive = Join-Path $RunsDir "radar_forever_$((Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssZ')).log"
    ...
    function Write-Log {
        ...
        Rotate-Log-IfNeeded
        Add-Content -LiteralPath $LogPath -Value "[$((Get-Date).ToString('yyyy-MM-ddTHH:mm:sszzz'))] $Message" -Encoding UTF8
    }
    ...
    function Write-Heartbeat {
        ...
        try {
            # Refreshing the owner PID also repairs races with the legacy direct launcher.
            ...
            Write-AtomicJson -Path $HeartbeatPath -Value ([ordered]@{
                ...
                child_entry_point = $RadarEntryPoint
                ...
                last_iteration_finished_at_utc = if ($FinishedAtUtc) { $FinishedAtUtc } else { $null }
                last_exit_code = $ExitCode
                last_error = $LastError
            })
        }
        catch {
            try { Write-Log "Heartbeat write failed: $($_.Exception.Message)" }
            catch {}
        }
    }

    Set-Content -LiteralPath $PidPath -Value $PID -Encoding ASCII
    Write-AtomicJson -Path $PidMetaPath -Value ([ordered]@{
        pid = $PID
        started_at_utc = $SupervisorStartedAt
        project_root = $ProjectRoot
    ...
    $ProviderEnvNames = @("OPENAI_API_KEY", "ANTHROPIC_API_KEY", "GEMINI_API_KEY", "GOOGLE_API_KEY", "MISTRAL_API_KEY", "GROQ_API_KEY", "OPENROUTER_API_KEY", "AZURE_API_KEY", "COHERE_API_KEY")
    $LoadedProviderKeys = @()
    foreach ($name in $ProviderEnvNames) {
        ...
        if ([Environment]::GetEnvironmentVariable($name, "Process")) { $LoadedProviderKeys += $name }
    }

    Write-Heartbeat -Status "starting"
    Write-Log "Radar supervisor started. pid=$PID root=$ProjectRoot python=$PythonExe entry_point=$RadarEntryPoint mutex=$MutexName litellm=$($env:RADAR_USE_LITELLM) provider_keys=$($LoadedProviderKeys -join ',')"
    ...
    while ($true) {
        ...
        $failureMessage = ""
        try {
            Write-Log "Starting one radar iteration."
            $child = Start-Process -FilePath $PythonExe `
                -ArgumentList @("-B", "`"$RadarEntryPoint`"", "--iterations", "1", "--interval", "$IterationIntervalSeconds") `
            ...
            do {
                Write-Heartbeat -Status "running_iteration" -IterationStartedAtUtc $iterationStartedAt `
            ...
            Write-Log "Radar iteration exited with code $exitCode. child_pid=$($child.Id)"
        }
        catch {
            $failureMessage = "Radar iteration failed: $($_.Exception.Message)"
        ...
        Write-Heartbeat -Status "sleeping" -FinishedAtUtc ((Get-Date).ToUniversalTime().ToString("o")) `
            -ExitCode $exitCode -LastError $failureMessage
