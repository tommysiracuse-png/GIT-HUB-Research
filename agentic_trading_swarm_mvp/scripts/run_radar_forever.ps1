...
@{ pid=$PID; started_at_utc=$SupervisorStartedAt; project_root="$ProjectRoot"; script="run_radar_forever.ps1" } | ConvertTo-Json | Set-Content -Path $PidMetaPath -Encoding UTF8
...
function Rotate-Log-IfNeeded {
    if (Test-Path $LogPath) {
        ...
        if ($item.Length -gt $MaxLogBytes) {
            ...
            $archive = Join-Path $RunsDir "radar_forever_$stamp.log"
...
Write-Log "Radar supervisor started. pid=$PID root=$ProjectRoot python=$PythonExe litellm=$($env:RADAR_USE_LITELLM) provider_keys=$($LoadedProviderKeys -join ',')"
...
while ($true) {
    ...
    try {
        ...
        $output = & $PythonExe -B (Join-Path $SrcDir "radar_loop.py") --iterations 1 --interval 60 2>&1
        $exitCode = $LASTEXITCODE
        foreach ($line in $output) {
            Add-Content -Path $LogPath -Value $line -Encoding UTF8
        }
        Write-Log "Radar iteration exited with code $exitCode."
    }
    catch {
        Write-Log "Radar iteration failed: $($_.Exception.Message)"
    }
    finally {
        Pop-Location
    }
