...
$EvolutionEntryPoint = (Resolve-Path -LiteralPath (Join-Path $SrcDir "evolution_worker.py")).Path
...
$ChildStdoutPath = Join-Path $RunsDir "evolution_worker_child_$PID.stdout.tmp"
$ChildStderrPath = Join-Path $RunsDir "evolution_worker_child_$PID.stderr.tmp"
$MaxLogBytes = 10MB
$IntervalSeconds = 300
...
function Stop-OwnedChildTree {
    ...
    if ($root -and ([string]$root.Name -notlike "python*.exe" -or
        ...
        -not (Test-ExactPathArgument -CommandLine ([string]$root.CommandLine) -ExpectedPath $EvolutionEntryPoint) -or
...
function Test-ExistingExactOwner {
    foreach ($process in @(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue)) {
        ...
        if ([string]$process.Name -like "python*.exe" -and
            (Test-ExactPathArgument -CommandLine ([string]$process.CommandLine) -ExpectedPath $EvolutionEntryPoint)) { return $true }
    }
    return $false
}

$MutexName = "Global\AgenticTradingSwarm.Evolution.$(Get-OwnershipHash)"
...
@{ pid=$PID; started_at_utc=$SupervisorStartedAt; project_root="$ProjectRoot"; script="run_evolution_worker_forever.ps1"; script_path=$RunnerPath; mutex_name=$MutexName } | ConvertTo-Json | Set-Content -Path $PidMetaPath -Encoding UTF8
...
function Write-Heartbeat {
    ...
    try {
        Write-AtomicJson -Path $HeartbeatPath -Value ([ordered]@{
            ...
            child_entry_point = $EvolutionEntryPoint
...
while ($true) {
    ...
    try {
        ...
        $child = Start-Process -FilePath $PythonExe `
            -ArgumentList @("-B", "`"$EvolutionEntryPoint`"", "--iterations", "1", "--interval", "$IntervalSeconds") `
        ...
        Write-Log "Evolution worker iteration exited with code $exitCode. child_pid=$($child.Id)"
    }
    catch {
        $failureMessage = "Evolution worker iteration failed: $($_.Exception.Message)"
        Write-Log $failureMessage
        if ($child) {
            # Do not clear the in-flight identity or begin another iteration until
            # the exact child we started is confirmed exited.
            while ($true) {
                try {
                    $child.Refresh()
